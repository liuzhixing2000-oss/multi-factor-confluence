...
def backtest(x,s):
    trades=[]; side=None; entry=stop=target=0.0
    for i in range(20,len(x)-1):
        if side:
            hi,lo=float(x.h.iloc[i]),float(x.l.iloc[i])
            hit=(side=="L" and lo<=stop) or (side=="S" and hi>=stop)
            win=(side=="L" and hi>=target) or (side=="S" and lo<=target)
            if hit or win or i>=exit_i:
                r=(stop-entry)/(entry-stop) if side=="L" else (entry-stop)/(stop-entry)
                if win and not hit: r=2.0
...
def run():
    try:
        for symbol in ["ETHUSDT","BTCUSDT","SOLUSDT"]:
            x=candles(symbol); m=x.resample("15min",closed="right",label="right").agg({"o":"first","h":"max","l":"min","c":"last","v":"sum"}).dropna(); h=x.resample("60min",closed="right",label="right").agg({"o":"first","h":"max","l":"min","c":"last","v":"sum"}).dropna()
            s=signals(h,m,x); causal_check(h,m,x); bt=backtest(x,s)
            emit({"event":"mtf_result","symbol":symbol,"bars_5m":len(x),"signals":int((s.long_signal|s.short_signal).sum()),"longs":int(s.long_signal.sum()),"shorts":int(s.short_signal.sum()),"causal_check":True,**bt})
